...
## 1.5 在工业界（特别是地图/定位）有哪些场景？

| 场景 | 状态 s | 动作 a | 奖励 r |
|-----|-------|--------|-------|
| **GNSS 修正** | 原始 GNSS + IMU + 上一时刻状态 | 位置/速度修正量 | 与高精地面真值的负距离 |
| **地图匹配 (RLOMM)** | 当前轨迹 embedding + 候选路段 | 选择哪条候选路 | 是否匹配上正确路段 |
| **路径规划** | 起点/终点/路网/实时路况 | 下一段路 | 时间/距离/路况权重 |
| **ETA 估计** | 历史时段 + 起点终点 | 估计时间值 | 与实际到达时间误差 |
| **传感器选择** | 各传感器置信度 | 选哪些传感器融合 | 融合精度 |
...
- 解释 "exploration vs exploitation"，举一个真实业务例子
